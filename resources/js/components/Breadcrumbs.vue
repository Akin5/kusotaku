<template>
	<div>
		<ol class="breadcrumb" v-if="$router">
			...
			<template v-if="!isInitialEmptyRoute">
...
<script>
// https://raw.githubusercontent.com/Suruat/vue-2-crumbs/master/src/breadcrumbComponent.js
const utils = {
	isObject(checkMe) {
		return (
			typeof checkMe === "object" && !Array.isArray(checkMe) && checkMe !== null
		);
	},
};
export default {
	...
	computed: {
		...
		currentRoute() {
			...
			if (
				!this.isInitialEmptyRoute &&
				(this.parentHelper || !this.parentHelper)
			) {
				return this.$route;
			}
		},
	...
	methods: {
		...
		getRootParentRoute(route) {
			...
			rootParentRoute = this.resolveRootParentRoute(
			...
			if (route.path === rootParentRoute.path) {
				rootParentRoute = this.resolveRootParentRoute(
					matchedRoutes[matchedRoutes.length - 3],
				);
			}
		...
		getDirectParentRoute(route) {
			...
			if (breadcrumb && breadcrumb.parent) {
				...
				if (typeof breadcrumbParent === "string") {
					...
					console.error(
						`Vue-2-Crumbs Error: 'parent' property in breadcrumb object for '${route.name}' route has wrong type. Only string or object is allowed`,
					);
				}
		...
		getAncestorsRoutesArray(route) {
			...
			if (parentRoute) {
				...
				parentRoutesArray = [
					...this.getAncestorsRoutesArray(parentRoute),
					routeObjectToAdd,
				];
			}
